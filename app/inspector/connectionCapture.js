...
function startFullConnectionCapture() {
  ...
  enqueue({ type: "INSPECTOR_START", source: "launcher", direction: "local", policy: "passive-no-network-modification", captureLevel: "application-plaintext-after-TLS", redaction: "proxy-credentials-redacted; controlled-backend-capture-enabled; metadata-only-for-third-party-profile-traffic", encryptionAtRest: "none", outputs: ["conexion", "conexion.ndjson", "launch.log", "bodies/*", "extension-load.ndjson", "extensions/original-downloads.ndjson", "profiles/<profileUniqueName>/profile.json", "profiles/<profileUniqueName>/conexion", "profiles/<profileUniqueName>/bodies/*"] })
